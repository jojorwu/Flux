...
Flux groups nearby loaded chunks to form an "independent region."
See [the PaperMC documentation](https://docs.papermc.io/flux/reference/region-logic) for exact details on how Flux
...
For a server with many spread out players, Flux will create many
spread out regions and tick them all in parallel on a configurable sized
threadpool. Thus, Flux should scale well for servers like this.

Flux is also its own project, this will not be merged into Paper
for the foreseeable future. 

A more detailed but abstract overview: [Project overview](https://docs.papermc.io/flux/reference/overview).

## FAQ

### What server types can benefit from Flux?
Server types that naturally spread players out, 
like skyblock or SMP, will benefit the most from Flux. The server
should have a sizeable player count, too.

### What hardware will Flux run best on?
Ideally, at least 16 _cores_ (not threads).

### How to best configure Flux?
...
done before Flux was released on the test server we ran that
...
in Flux. Additionally, multithreading of _any kind_ introduces
...
be compatible with Flux. However, there are plans to add API that 
would allow Flux plugins to be compatible with Paper.

For example, the Bukkit Scheduler. The Bukkit Scheduler inherently
relies on a single main thread. Flux's RegionScheduler and Flux's
...
First, Flux breaks many plugins. To aid users in figuring out which
plugins work, only plugins that have been explicitly marked by the
author(s) to work with Flux will be loaded. By placing
"flux-supported: true" into the plugin's plugin.yml, plugin authors
...
This means that Flux compatible plugins need to take advantage of 
...
[Project overview](https://docs.papermc.io/flux/reference/overview).
